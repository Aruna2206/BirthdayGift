...
export function AdminPage() {
  ...
  const [token, setToken] = useState<string | null>(null); // Start with null to force re-check after mount
  const [username, setUsername] = useState('mylove');
  const [password, setPassword] = useState('forever');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-400 via-rose-400 to-red-400 relative overflow-hidden text-gray-800">
      ...
      >
        ...
        <p className="text-center mt-3 text-rose-500 font-bold tracking-wide">Upload Time!</p>
      </motion.div>
          ...
          >
            ...
            <div className="relative flex items-center justify-center bg-gradient-to-br from-pink-500 via-rose-500 to-red-500 p-3 rounded-full shadow-xl overflow-hidden">
              ...
              <div className="absolute inset-0 flex items-center justify-center pt-1 pr-[1px]">
                  <Sparkles className="text-yellow-300 group-hover/card:scale-110 transition-transform duration-300" size={18} fill="currentColor" />
              </div>
            </div>
          </motion.div>
        </motion.div>
        ...
        >
          ...
          {error && (
            <motion.div
              ...
              className="bg-red-100 border-2 border-red-400 text-red-700 px-5 py-4 rounded-2xl text-sm font-medium text-center"
            >
              {error}
            </motion.div>
          )}
          ...
          <motion.button
            ...
            type="submit"
            className="w-full bg-gradient-to-r from-pink-500 via-rose-500 to-red-500 text-white py-4 rounded-2xl font-bold text-base shadow-xl hover:shadow-2xl transition-all flex items-center justify-center gap-2 relative overflow-hidden group disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {loading ? (
